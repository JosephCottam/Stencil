...
public class StencilTreeAdapter extends CommonTreeAdaptor {
	...
	public Object create(Token token) {
		...
		case 55	: t = new Layer(token); break;		//Token LAYER
		case 74	: t = new LT(token); break;		//Token LT
		//Default for CONST
		//Default for GUIDE_SUMMARIZATION
		//Default for GUIDE_DIRECT
		case 70	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: SEPARATOR");
		...
		case 47	: t = new Canvas(token); break;		//Token CANVAS
		case 54	: t = new Import(token); break;		//Token IMPORT
		case 8	: t = new CallChain(token); break;		//Token CALL_CHAIN
		case 68	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: ARG");
		case 69	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: CLOSE_ARG");
		//Default for BASE
		//Default for GUIDE_YIELD
		case 56	: t = new Last(token); break;		//Token LAST
		case 76	: t = new EQ(token); break;		//Token EQ
		case 99	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: COMMENT");
		case 64	: t = new Stream(token); break;		//Token STREAM
		//Default for GATE
		...
		case 65	: t = new View(token); break;		//Token VIEW
		case 67	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: CLOSE_GROUP");
		case 61	: t = new Prefilter(token); break;		//Token PREFILTER
		case 58	: t = new Null(token); break;		//Token NULL
		case 20	: t = new StencilNumber(token); break;		//Token NUMBER
		//Default for OPERATOR_INSTANCE
		case 57	: t = new Local(token); break;		//Token LOCAL
		case 18	: t = new List(token); break;		//Token LIST
		//Default for NAMESPACE
		case 7	: t = new Consumes(token); break;		//Token CONSUMES
		case 86	: t = new Yields(token); break;		//Token YIELDS
		//Default for TUPLE_VALUE
		//Default for ANIMATED_DYNAMIC
		case 37	: t = new StateQuery(token); break;		//Token STATE_QUERY
		//Default for GROUP
		case 98	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: WS");
		case 25	: t = new OperatorRule(token); break;		//Token OPERATOR_RULE
		//Default for FILTER
		//Default for TAGGED_ID
		case 72	: t = new GT(token); break;		//Token GT
		//Default for FROM
		...
		//Default for FOLD
		//Default for FACET
		case 78	: t = new RE(token); break;		//Token RE
		case 9	: t = new CanvasDef(token); break;		//Token CANVAS_DEF
		case 60	: t = new Order(token); break;		//Token ORDER
		//Default for ANNOTATION
		case 95	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: DIGITS");
		case 73	: t = new GTE(token); break;		//Token GTE
		...
		case 91	: t = new Id(token); break;		//Token ID
		//Default for GUIDE_GENERATOR
		//Default for DEFINE
		//Default for DIRECT_YIELD
		case 75	: t = new LTE(token); break;		//Token LTE
		case 30	: t = new Predicate(token); break;		//Token PREDICATE
		case 97	: throw new IllegalArgumentException("Attempted to create tree-node for token on error list: ESCAPE_SEQUENCE");
		//Default for AS
		case 39	: t = new Selector(token); break;		//Token SELECTOR
		//Default for ISLAND_BLOCK
		case 53	: t = new Guide(token); break;		//Token GUIDE
		case 42	: t = new TupleFieldDef(token); break;		//Token TUPLE_FIELD_DEF
		case 62	: t = new Python(token); break;		//Token PYTHON
		case 44	: t = new All(token); break;		//Token ALL
		//Default for ANIMATED
		case 79	: t = new NRE(token); break;		//Token NRE
		//Default for NESTED_BLOCK
		case 4	: t = new AstInvokeable(token); break;		//Token AST_INVOKEABLE
		case 59	: t = new Operator(token); break;		//Token OPERATOR
		//Default for DEFAULT
		case 34	: t = new Result(token); break;		//Token RESULT
		//Default for TAG
		case 77	: t = new NEQ(token); break;		//Token NEQ
		...
		//Default for BASIC
		case 94	: t = new StencilString(token); break;		//Token STRING
