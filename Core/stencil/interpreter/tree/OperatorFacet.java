...
public class OperatorFacet implements Viewpoint<OperatorFacet> {
	...
	public OperatorFacet viewpoint() {
		...
		final Rule[] vpp = new Rule[prefilters.length];
		for (int i=0; i<vpp.length; i++) {vpp[i] = prefilters[i].viewpoint();}

		return new OperatorFacet(name, arguments, results, vpp, vpr);
	}
}
